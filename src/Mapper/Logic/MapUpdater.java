...
public final class MapUpdater {
    ...
    public static void update() {
        ...
        final double realSize = (double)Display.getHeight()/(double)Properties.scale/4d;
        final double tileSize = ((double)Display.getHeight()/(double)Properties.scale);

        final int tileX = (int) (((double)MouseInput.x+(double)UpCamera.x*realSize)/((double)Display.getWidth()/(double)Properties.scale/tileScaler))+1;
        final int tileY = (int) (((double)MouseInput.y+(double)UpCamera.y*realSize)/((double)Display.getHeight()/(double)Properties.scale));
